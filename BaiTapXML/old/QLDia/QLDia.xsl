    ...
    <xsl:template match="/">
        ...
        <HTML>
            ...
            <BODY>
                ...
                <h2>Chọn các Track có length be hon 5 phut</h2> <xsl:value-of select="/SPECIALS/CD/TRACK[@length&lt;=5]"/>
                <h2>Tim cac dia CD co ten bat dau la A</h2> <xsl:value-of select="//CD[starts-with(normalize-space(),'A')]"/>
                <h2>Tim cac dia CD co ten chua chu "hihi"</h2> <xsl:value-of select="//CD[contains(self::*, 'hihi')]"/>
                <h2>Tim cac dia CD co ten khong chua chu "Xuan"</h2> <xsl:value-of select="//CD[contains(self::*, 'hihi') = false()]"/>
                ...
                <h2>Chọn các phần tử cùng cấp sau phần tử CD đầu tiên</h2>
                <h2>Chọn các phần tử theo su phần MEsage và các phần tử con của nó</h2>
