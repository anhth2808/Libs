...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        <xsl:for-each select="DanhSachKhachSan/KhachSan/KhachHang[@MaKH=$MAKHACHHANG1]">
          <font color="black">
            <h4 align="left">
      ...
      min="2018-01-01" max="2028-12-31" />
        ...
        <hr> </hr>
           <xsl:for-each select="DanhSachKhachSan/KhachSan/KhachHang[@MaKH=$MAKHACHHANG2]">
          <font color="black">
            <h4 align="left">
              Mã Khách Hàng : <xsl:value-of select="MaKH"/>
            </h4>
      ...
      min="2018-01-01" max="2028-12-31" />
          ...
          <xsl:for-each select="HoaDon">
            ...
            <xsl:variable name="mauLoaiPhong">
              <xsl:if test=" LoaiPhong='Doi'">#4682B4</xsl:if>
            </xsl:variable>

            <TABLE align="center"  border="1">
              <tr bgcolor="{$mauLoaiPhong}">
                <TH WIDTH= "30" HEIGHT= "5">
                  <xsl:number value="position()" format="1"/>
                </TH>
                <TH WIDTH= "150" HEIGHT= "23">
